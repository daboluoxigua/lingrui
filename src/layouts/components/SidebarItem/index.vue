...
<template>
  <div class="sidebar-item">
    ...
    >
      ...
      >
        <el-menu-item :title="item.meta?.title ?? '[ 无标题 ]'" :index="resolveRoutePath(basePath, item.path || '')">
          <i v-if="item.meta?.icon" :class="item.meta.icon" class="title-icon"></i>
    ...
    <el-sub-menu
      ...
      :index="
        settingsStore.settings.app.routeBaseOn !== 'filesystem' ? resolveRoutePath(basePath, item.path) : JSON.stringify(item)
      "
    >
      <template #title>
        <i v-if="item.meta?.icon" :class="item.meta.icon" class="title-icon"></i>
