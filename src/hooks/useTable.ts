...
export function useTable(
  ...
  initParam: object = {},
  isPageable = true,
  dataCallBack?: (data: any, param: any) => any,
  ...
  const getData = async () => {
    try {
      ...
      const param = { ...initParam, ...state.totalParam, ...state.searchInitParam };
      let data = await api(param);
      dataCallBack && (data = dataCallBack(data, param));

      state.tableData = isPageable ? data.records : data;
      // 解构后台返回的分页数据 (如果有分页更新分页信息)
      const { current: pageIndex, size: pageSize, total } = data;
      isPageable && updatePageable({ pageIndex, pageSize, total });
    } catch (error) {
