...
const useRouteStore = defineStore(
  ...
  () => {
    ...
    const routesHeader = ref<Route.recordMainRaw[]>([]);
    ...
    // 格式化后端路由数据
    function getAsyncRoutes(routes: any, views = import.meta.glob("../../views/**/*.vue")): Route.recordMainRaw[] {
      const routeArr: any[] = [];
      ...
      routes.forEach((item: any) => {
        ...
        const newItem: any = {
          meta: {
            ...
            sidebar: item.isHidden,
            breadcrumb: item.breadcrumb,
          },
        };
        if (item.menuNameEn) {
          newItem.name = item.menuNameEn;
        }
        newItem.path = item.url;
        if (item.component) {
          if (item.component === "Layout") {
            newItem.component = () => import("@/layouts/index.vue");
          } else if (item.component) {
            newItem.component = views[`../../views/${item.component}`];
          }
        }
        ...
        if (newItem.children && newItem.children.length > 0 && newItem.children[0].type === "menu") {
          // 只有一个下级菜单时 需要提升为一级菜单 父级的redirect 指向子级 子级隐藏 并且activeMenu为父级
          newItem.children[0].activeMenu = newItem.url;
          newItem.redirect = newItem.children[0].url.startsWith("/")
            ? newItem.children[0].url
            : `${newItem.path}/${newItem.children[0].url}`;
        }
    ...
    async function generateRoutesAtBack() {
      ...
      await api
        ...
        .then(async (res: any) => {
          routesHeader.value = res.find((item) => item.menuModule === "lingrui-tenant-service-sys");
          // 设置 routes 数据
          if (!!routesHeader.value) {
            routesRaw.value = getAsyncRoutes(routesHeader.value.children);
          }
          // routesRaw.value = formatBackRoutes(routesHeader.value.children);
